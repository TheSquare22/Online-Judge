...
type TagFormDefaults = Pick<NewTag, 'id' | 'visible'>;

type TagFormGroupContent = {
  id: FormControl<ITag['id'] | NewTag['id']>;
  title: FormControl<ITag['title']>;
  keywords: FormControl<ITag['keywords']>;
  visible: FormControl<ITag['visible']>;
};
...
export class TagFormService {
  createTagFormGroup(tag: TagFormGroupInput = { id: null }): TagFormGroup {
    ...
    return new FormGroup<TagFormGroupContent>({
      ...
      keywords: new FormControl(tagRawValue.keywords),
      visible: new FormControl(tagRawValue.visible),
    });
  }
