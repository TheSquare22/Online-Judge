...
type RestOf<T extends ISubmission | NewSubmission> = Omit<T, 'uploadDate'> & {
...
export class SubmissionService {
  ...
  protected convertDateFromClient<T extends ISubmission | NewSubmission | PartialUpdateSubmission>(submission: T): RestOf<T> {
    return {
      ...submission,
      uploadDate: submission.uploadDate?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restSubmission: RestSubmission): ISubmission {
    return {
      ...restSubmission,
      uploadDate: restSubmission.uploadDate ? dayjs(restSubmission.uploadDate) : undefined,
    };
  }
